...
class JAISAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.register_buffer( # torch.Size([1, 1, 2048, 2048]) lower triangular  matrix with True
            "bias",
    ...
    def forward(
        ...
        if layer_past is not None: # NOTE they are updating the cache here, Have to understand what happens here 
        ...
        if self.reorder_and_upcast_attn:
            ...
            attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask, position_bias) # NOTE complex stuff : have to underssndn 
...
class JAISBlock(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        inner_dim = config.n_inner if config.n_inner is not None else 4 * hidden_size # The ratio is 2.66 instead of the general 4 - maybe model was too big
        ...
        self.adapter_down =  torch.nn.Parameter(torch.zeros([hidden_size,hidden_size//64]).fill_diagonal_(1))
        self.adapter_swigu = nn.GELU()
        self.adapter_up = torch.nn.Parameter(torch.zeros([hidden_size//64,hidden_size]).fill_diagonal_(1))
    ...
    def forward(
        ...
        adapter_state = self.adapter_ln(hidden_states)
        adapter_state = adapter_state @ self.adapter_down
        adapter_state = self.adapter_swigu(adapter_state)
        adapter_state = adapter_state @ self.adapter_up
        
        hidden_states = hidden_states + adapter_state
...
# NOTE: Just a check to see if correct shape of input embeddings is given
        ...
        if past_key_values is None:
            past_length = 0
            past_key_values = tuple([None] * len(self.h)) # NOTE: List with None filled to a length of num_layers
        else:
        ...
        # JAISAttention mask.
        if attention_mask is not None: # <- This is skipped 
            if batch_size <= 0:
            ...
            attention_mask = attention_mask[:, None, None, :] # NOTE SHAPE OF ATTEINOT MASK IS CHANGED TO 2,X -> 2,1,1,X
            ...
            attention_mask = (1.0 - attention_mask) * torch.finfo(self.dtype).min # NOTE THAT attention mask is now 0 and not the expected 1
        ...
        if self.config.add_cross_attention and encoder_hidden_states is not None:
            ...
            encoder_attention_mask = None # <- This is executed 
        ...
        if inputs_embeds is None:
            inputs_embeds = self.wte(input_ids) # We get the Word embeddings here
        if self.wpe is not None:
        ...
        ) # NOTE : IDK why, but we multiply the embeddings by this scalar
        ...
        if self.relative_pe is not None:
            ...
            position_bias = self.relative_pe(length, length, cached_kv_length) # NOTE Have to understand what this
        else:
            position_bias = None
...
class JAISLMHeadModel(JAISPreTrainedModel):
    ...
    def forward(
        ...
        """
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict # HAVE TO KNOW WHY RETURN DICT IS TRUE AND WHAT IT DOES

        transformer_outputs = self.transformer(
            input_ids,
            past_key_values=past_key_values,
            attention_mask=attention_mask, # WE HAVE TO PASS THE ATTENTION MASK FROM OUTSIDE , THAT CAN BE DONE BY self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            token_type_ids=token_type_ids, # 
            position_ids=position_ids, # SAME THING FOR POSTIION ID ALSO 
            head_mask=head_mask,
            inputs_embeds=inputs_embeds,
            encoder_hidden_states=encoder_hidden_states,
            encoder_attention_mask=encoder_attention_mask,
            use_cache=use_cache, # THIS IS TRUE
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict, # ONLY THIS IS TRUE
        )
